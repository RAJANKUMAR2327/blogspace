...
export default function Home() {
  const { data: blogsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await blogAPI.getAll({ limit: 6, status: 'published' })
      return res.data
    }
  })
  ...
  const { data: trendingData } = useQuery({
    ...
    queryFn: async () => {
      const res = await blogAPI.getAll({ limit: 4, sort: 'views', status: 'published' })
      return res.data
    }
  })
  ...
  return (
    <div>
      <SEO title="Home" description="Discover stories and ideas from expert writers" />
      ...
      <section className="relative bg-gradient-to-br from-purple-50 via-white to-indigo-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-800 py-24 px-4 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/blogs" className="inline-flex items-center justify-center gap-2 bg-purple-600 text-white px-8 py-4 rounded-full text-lg font-medium hover:bg-purple-700 transition-colors shadow-lg shadow-purple-200 dark:shadow-none">
              <FiBookOpen /> Start Reading
            </Link>
            <Link to="/register" className="inline-flex items-center justify-center gap-2 border-2 border-purple-200 dark:border-purple-700 text-purple-700 dark:text-purple-300 px-8 py-4 rounded-full text-lg font-medium hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
        ...
        <section className="max-w-7xl mx-auto px-4 pb-16">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <FiTrendingUp className="text-purple-600" /> Trending
            </h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {trendingData.blogs.map((blog, i) => (
              <Link key={blog._id} to={`/blog/${blog.slug}`}
                className="flex gap-4 p-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 hover:shadow-md hover:-translate-y-0.5 transition-all">
                <span className="text-3xl font-bold text-purple-100 dark:text-purple-900 w-10 flex-shrink-0 leading-none">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <div>
                  <span className="text-xs font-semibold text-purple-600 dark:text-purple-400 uppercase">{blog.category}</span>
                  <h3 className="font-bold text-gray-900 dark:text-white line-clamp-2 hover:text-purple-600 transition-colors">
                    {blog.title}
                  </h3>
                  <p className="text-xs text-gray-400 mt-1 flex items-center gap-2">
                    <FiZap size={10} /> {blog.views} views · {blog.readTime}m read
                  </p>
                </div>
              </Link>
            ))}
