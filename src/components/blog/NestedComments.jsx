...
function CommentItem({ comment, blogId, depth = 0 }) {
  ...
  return (
    <div style={{ marginLeft: depth > 0 ? 32 : 0 }}>
      <div style={{ background: depth === 0 ? '#0d0d1a' : 'rgba(124,58,237,0.05)', border: `1px solid ${depth === 0 ? 'rgba(255,255,255,0.06)' : 'rgba(124,58,237,0.1)'}`, borderRadius: 12, padding: '16px 18px', marginBottom: 8 }}>
        {/* Header */}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {user && depth === 0 && (
              <button onClick={() => setShowReply(!showReply)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.25)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, transition: 'color 0.2s', fontFamily: "'Inter',sans-serif" }}
            ...
            {(user?._id === comment.user?._id || user?.role === 'admin') && (
              <button onClick={() => deleteMutation.mutate(comment._id)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.2)', fontSize: 14, display: 'flex', alignItems: 'center', transition: 'color 0.2s' }}
  ...
  return (
    <div style={{ fontFamily: "'Inter',sans-serif" }}>
      <h2 style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 700, color: '#fff', marginBottom: 24, letterSpacing: '-0.3px' }}>
        Discussion <span style={{ fontSize: 15, color: 'rgba(255,255,255,0.25)', fontWeight: 400 }}>({comments?.length || 0})</span>
      </h2>
      ...
      {user ? (
        <div style={{ background: '#0d0d1a', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: 18, marginBottom: 24 }}>
          <textarea
            ...
            rows={3}
            style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, fontSize: 14, color: '#fff', outline: 'none', fontFamily: "'Inter',sans-serif", resize: 'none', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
            onFocus={e => e.target.style.borderColor = 'rgba(167,139,250,0.4)'}
            onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.08)'}
          />
